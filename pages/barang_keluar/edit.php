<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item_name == '' || $qty == '' || $item_location == '') {
      ...
      $stmt = $pdo->prepare("UPDATE barang_keluar SET nama_barang = ?, qty = ?, lokasi_barang = ?, id_kategori = ?, id_pemasok = ? WHERE id_barang_keluar = ?");
      $stmt->execute([$item_name, $qty, $item_location, $id_category, $id_supplier, $id]);
    }

    // Menampilkan alert dan redirect ke index
    echo "<script type='text/javascript'>alert('Barang keluar berhasil diubah'); window.location.href='index.php';</script>";
  }
?>
<div class="content flex-column">
    <h2 class="mb-8">Edit Barang Keluar</h2>
    <p class="small">Halaman untuk mengubah data barang keluar</p>
    <hr class="mt-20 mb-32">
    <form class="flex-column gap-16 w-50" action="edit.php?id=<?php echo $barang_keluar['id_barang_keluar']; ?>" method="post">
      <div class="flex-column gap-4">
        <label class="small" for="item_name">Nama Barang</label>
        <input type="text" id="item_name" name="item_name" value="<?php echo htmlspecialchars($barang_keluar['nama_barang']); ?>" required>
      </div>
      <div class="flex-column gap-4">
        <label class="small" for="qty">Qty</label>
        <input type="number" id="qty" name="qty" value="<?php echo htmlspecialchars($barang_keluar['qty']); ?>" required>
      </div>
      <div class="flex-column gap-4">
        <label class="small" for="item_location">Lokasi Barang</label>
        <input type="text" id="item_location" name="item_location" value="<?php echo htmlspecialchars($barang_keluar['lokasi_barang']); ?>" required>
      </div>
      <div class="flex-column gap-4">
        <label class="small" for="id_category">Kategori Barang</label>
        <select name="id_category" id="id_category" required>
          <option selected value="<?php echo htmlspecialchars($barang_keluar['id_kategori']); ?>"><?php echo htmlspecialchars($barang_keluar['nama_kategori']); ?></option>
          <?php foreach ($categories as $category): ?>
            <?php if($category['id_kategori'] != $barang_keluar['id_kategori']): ?>
      ...
      <div class="flex-column gap-4">
        ...
        <select name="id_supplier" id="id_supplier" required>
          <option selected value="<?php echo htmlspecialchars($barang_keluar['id_pemasok']); ?>"><?php echo htmlspecialchars($barang_keluar['nama_pemasok']); ?></option>
          <?php foreach ($suppliers as $supplier): ?>
            <?php if($supplier['id_pemasok'] != $barang_keluar['id_pemasok']): ?>
